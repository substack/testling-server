...
module.exports = function (self, opts) {
    ...
    ci.on('commit', function (commit) {
        ...
        fs.readFile(pkgfile, function (err, src) {
            ...
            try { var browsers = normalizeBrowsers(pkg.testling.browsers) }
            catch (err) {
                return self.put({
                    type: 'error',
                    job: commit.id,
...
console.log('prepare err=', err);
                if (err) return;
                runTest(putter, commit.dir, function (err, href) {
console.log('ERR=', err, 'HREF=', href);
                    if (!err) visit(href);
                });
            });
        });
