            ...
            prepare(putter, commit.dir, function (err) {
                ...
                compile(putter, commit.dir, function (err, html) {
                    ...
                    var stack = Object.keys(browsers).reduce(function (acc, name) {
                        return acc.concat(browsers[name].reduce(function (v) {
                            return [ name, v ];
                        }));
                    }, []);
                    
                    var server = createServer(putter, html, commit.dir);
                    server.listen(0, function next () {
                        var addr = 'http://localhost:' + server.address().port;
                        if (stack.length === 0) {
                            return putter({ type: 'finished' });
                        }
                        var b = stack.shift();
                        server.on('finish', next);
                        
                        visit(addr + '?browser=' +  b[0]x '&version=' +  b[1]);
                    });
                });
            });
        });
